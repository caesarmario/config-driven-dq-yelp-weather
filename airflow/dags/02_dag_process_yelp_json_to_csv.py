####
## Airflow DAG: To process yelp json data to csv (Q4)
...
# -- DAG-level settings
job_name        = "process_yelp_json_to_csv"
...
def run_processor(file_name: str, **kwargs):
    ...
    # Build command
    cmd = [
        "python", "scripts/process_yelp_json_to_csv.py",
        "--file_name", file_name,
        "--creds", minio_creds
    ]
...
def run_merger(file_name: str, **kwargs):
    ...
    # Build command
    cmd = [
        "python", "scripts/merge_yelp_csv_to_minio.py",
        "--file_name", file_name,
        "--creds", minio_creds
    ]
