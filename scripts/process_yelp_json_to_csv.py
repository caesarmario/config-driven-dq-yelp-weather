...
class ProcessYelpJson:
    ...
    def __init__(self, file_name:str, credentials: dict):
        try:
            ...
            self.minio_output_path = f"yelp_dataset_csv/{file_name}"
    ...
    def process(self):
        ...
        try:
            ...
            with pd.read_json(local_json_path, lines=True, chunksize=self.chunk_size) as reader:
                for chunk in reader:
                    ...
                    try:
                        ...
                        output_file_name = f"{self.file_name.replace('.json', '')}_{chunk_id}.csv"
                        local_output_path = output_dir / output_file_name
                        validated_df.to_csv(local_output_path, index=False)
                        ...
                        if not invalid_df.empty:
                            ...
                            rejected_file_name = f"{self.file_name.replace('.json', '')}_rejected_{chunk_id}.csv"
            ...
            if output_dir.exists():
                ...
                logger.info(f">> Removed local csv output directory: {output_dir}")
